...
bot.action("taqvim", async (ctx) => {
  await ctx.scene.enter("taqvimScene");
});

bot.action("duolar", async (ctx) => {
  await ctx.editMessageText(
    `<b>🤲 DUOLAR\n\n` +
      `1️⃣ Saharlik duosi\n` +
      `2️⃣ Iftorlik duosi\n` +
      `3️⃣ Taroveh tasbixi</b>`,
    {
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [
          [
            { text: "🌅 Saharlik", callback_data: "duo_sahar" },
            { text: "🌆 Iftorlik", callback_data: "duo_iftor" },
          ],
          [{ text: "📿 Taroveh", callback_data: "duo_taroveh" }],
          [{ text: "🔙 Ortga", callback_data: "restartBot" }],
        ],
      },
    }
...
bot.action("duo_sahar", async (ctx) => {
  await ctx.editMessageText(
    `<b>🌅 SAHARLIK DUOSI\n\n` +
      `Navaytu an asuma sovma shahri Ramazona minal fajri ilal mag'ribi, ` +
      `xolisan lillahi ta'ala. Allohu akbar!\n\n` +
      `Ma'nosi: Ramazon oyining ro'zasini tong otgandan kun botgunigacha ` +
      `xolis Alloh uchun tutishni niyat qildim. Allohu akbar.</b>`,
    {
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [
          [{ text: "🔙 Ortga", callback_data: "duolar" }],
          [{ text: "🏠 Bosh menyu", callback_data: "restartBot" }],
        ],
      },
    }
...
bot.action("duo_iftor", async (ctx) => {
  await ctx.editMessageText(
    `<b>🌆 IFTORLIK DUOSI\n\n` +
      `Allohumma laka sumtu va bika amantu va a'alayka tavakkaltu ` +
      `va 'ala rizqika aftartu, fag'firli, ya G'offaru, ma qoddamtu vama axxortu.\n\n` +
      `Ma'nosi: Yo Alloh, Senga imon keltirib, Senga tavakkal qilib, Sen uchun ro'za tutdim. ` +
      `Sen bergan rizq bilan iftor qildim. Ey gunohlarni kechirguvchi Zot, ` +
      `oldingi va keyingi gunohlarimni kechir. Omin!</b>`,
    {
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [
...
bot.action("duo_taroveh", async (ctx) => {
  await ctx.editMessageText(
    `<b>📿 TAROVEH TASBIXI\n\n` +
      `Subhaana zil mulki val malakut. Subhaana zil 'izzati val 'azomati val qudroti ` +
      `val kibriyai val jabarut. Subhaanal malikil hayyillaziy laa yanaamu va laa yamut. ` +
      `Subbuhun quddusur Robbunaa va Robbul malaaikati var Ruh. ` +
      `Laa ilaaha illallohu nastag'firulloh. Nas'alukal jannata va na'uzu bika minan nar.</b>`,
    {
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [
